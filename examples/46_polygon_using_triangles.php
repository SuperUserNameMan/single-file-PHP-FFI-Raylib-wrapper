...
// Draw a textured polygon, as a triangles fan
...
{
	if ( RL_USES_OPENGL_VERSION == 1 )
	{
		// XXX something fishy. Original example claims that only RLGL_QUADS support texturing
		// XXX but if you move rlSetTexture() just after the rlBegin(), then triangles are textured too.
		// XXX however, if raylib is compiled for OpenGL1.1, it does the oposite.
		RL_rlSetTexture( $texture->id );
	}

	RL_rlBegin( RLGL_TRIANGLES );

		if ( RL_USES_OPENGL_VERSION != 1 ) // XXX is this a valid workaround on all GPU and OS ?
		{
			RL_rlSetTexture( $texture->id );
		}
